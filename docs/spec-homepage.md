...
- Motion: use-case copy remains fully opaque but uses Apple-style scroll-linked horizontal parallax inside its card. The measured Apple caption container offsets by roughly `162px` per card of gallery progress while the card itself moves about `1277px`; LifeNotes should use the equivalent `160px` compensation per slide. Update this transform continuously from native scroll progress, not through a delayed class transition after snapping. Bottom dotnav should read as one active capsule moving through the dot row, with neighboring dots smoothly making room for it.
...
- Media lifecycle: when a use-case stops being current, immediately restore its static poster and pause it, but defer source removal and media reset until browser idle time. Keep only the current and next videos prepared; release all other HEVC/HDR decoders and frame buffers during idle maintenance.
...
- Carousel controls occupy a dedicated lane below the `680px` card instead of overlaying the device. Apple's authored cards reserve a lower safe area for the control; because LifeNotes devices extend to the card bottom, the equivalent non-obscuring treatment is an external control lane.
