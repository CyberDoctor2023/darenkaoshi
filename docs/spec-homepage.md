...
- Motion: use-case copy remains fully opaque and moves with its card through the native horizontal scroll, matching the measured Apple gallery behavior. Do not add a second delayed copy transition after the card reaches its snap point. Bottom dotnav should read as one active capsule moving through the dot row, with neighboring dots smoothly making room for it.
